...
class Case1BatchSolver:
    ...
    def __init__(self, m: float, n: float, tolerance: float = 1):
        """
        初始化Case1求解器
    ...
    def solve(self, combinations: np.ndarray) -> np.ndarray:
        ...
        Returns:
            np.ndarray: 解数组 (N_cbn, 5) - 每行为[xmin, xmax, ymin, ymax, phi]
                       无解的组合用np.inf填充
        """
        self.logger.debug(f"开始Case1批处理求解，组合数量: {len(combinations)}")
        ...
        self.logger.info(f"Case1求解完成，找到 {len(sols)} 个解，分布到 {np.sum(sol_cbn[:, 4] != np.inf)} 个组合")  # phi在第4列
        return sol_cbn
    ...
    def _expand_combinations(self, combinations: np.ndarray) -> tuple:
        ...
        self.logger.debug(f"组合扩展: {N_cbn} -> {len(expanded)}，保留原始编号追踪")
    ...
    def _compute_ab_coefficients(self, expanded_combinations: np.ndarray) -> tuple:
        ...
        self.logger.debug(f"A、B系数计算完成: ab_h{ab_h.shape}, ab_v{ab_v.shape}")
        return ab_h, ab_v
    ...
    def _compute_phi_candidates(self, ab_h: np.ndarray, ab_v: np.ndarray) -> tuple:
        ...
        self.logger.debug(f"生成phi候选: 水平{valid_count_h}个, 竖直{valid_count_v}个有效值")
        return phi_h, phi_v
    ...
    def _compute_collision_triangles_h(self, phi_h: np.ndarray, 
        ...
        self.logger.debug(f"计算水平边碰撞三角形: {len(colli_h)}个有效三角形")
    ...
    def _compute_collision_triangles_v(self, phi_v: np.ndarray, 
        ...
        self.logger.debug(f"计算竖直边碰撞三角形: {len(colli_v)}个有效三角形")
    ...
    def _solve_both_cases(self, colli_h: np.ndarray, colli_v: np.ndarray,
        ...
        # 处理水平边情况
        sols_h, indice_h = self._solve_horizontal_cases(colli_h, key_h, valid_indice_cbo_h, phi_h)
        if len(sols_h) > 0:
            sols_list.append(sols_h)
            indice_list.append(indice_h)
        
        # 处理竖直边情况
        sols_v, indice_v = self._solve_vertical_cases(colli_v, key_v, valid_indice_cbo_v, phi_v)
        if len(sols_v) > 0:
            sols_list.append(sols_v)
            indice_list.append(indice_v)
        
        # 合并结果
        if len(sols_list) == 0:
            return np.array([]).reshape(0, 5), np.array([])  # 改为5列
        
        sols = np.vstack(sols_list)
        indice_sol = np.concatenate(indice_list)
        
        return sols, indice_sol
    
    def _solve_horizontal_cases(self, colli_h: np.ndarray, key_h: np.ndarray,
                               valid_indice_cbo_h: np.ndarray, phi_h: np.ndarray) -> tuple:
        """
        求解水平边情况
        ...
        for i in range(len(colli_h)):
            ...
            # 找到对应的phi值 (需要从phi_h中找到对应的)
            # 这里简化处理，使用三角形重心的角度
            phi = np.arctan2(np.mean(triangle[:, 1]), np.mean(triangle[:, 0]))
            
            sol = self._solve_horizontal_case(triangle, t1, t2, t3, phi)
            # 不管解是否有效，都添加到列表中（无效解用np.inf标记）
        ...
        return np.array(sols_list), np.array(indice_list)
    
    def _solve_vertical_cases(self, colli_v: np.ndarray, key_v: np.ndarray,
                             valid_indice_cbo_v: np.ndarray, phi_v: np.ndarray) -> tuple:
        """
        求解竖直边情况
    ...
    def _solve_horizontal_case(self, triangle: np.ndarray, t1: float, t2: float, t3: float,
        ...
        if (abs(t1) > self.n + self.tolerance or 
            ...
            return np.array([np.inf, np.inf, np.inf, np.inf, np.inf])  # 用np.inf标记无效解
        ...
        if abs(abs(t1) - self.n) <= self.tolerance:
            ...
            x_center_max = self.m - min(xp0, xp1, xp2)
            x_center_min = -max(xp0, xp1, xp2)
            x_center = (x_center_max + x_center_min) / 2  # 取中点
        else:
            # 判断t2, t3符号是否一致
        ...
        return np.array([xmin, xmax, ymin, ymax, phi])  # 返回 [xmin, xmax, ymin, ymax, phi]
    ...
    def _solve_vertical_case(self, triangle: np.ndarray, t1: float, t2: float, t3: float,
        ...
        if abs(abs(t1) - self.m) <= self.tolerance:
            ...
            y_center_max = self.n - min(yp0, yp1, yp2)
            y_center_min = -max(yp0, yp1, yp2)
            y_center = (y_center_max + y_center_min) / 2  # 取中点
        else:
            # 判断t2, t3符号是否一致
            if (t2 > 0 and t3 > 0) or (t2 < 0 and t3 < 0):
                # 符号一致
    ...
    def _organize_solutions(self, sols: np.ndarray, indice_sol: np.ndarray, N_cbn: int) -> np.ndarray:
        ...
        sol_cbn = np.full((N_cbn, 5), np.inf)  # 改为5列：[xmin, xmax, ymin, ymax, phi]
        
        # 将解按组合编号放入对应位置
        for i, combo_idx in enumerate(indice_sol):
            if combo_idx < N_cbn:  # 防止索引越界
                sol_cbn[combo_idx] = sols[i]
